...
import { type SkinSmoothingSettings } from '../types';

interface SkinSmoothingSettingsPanelProps {
  settings: SkinSmoothingSettings;
  onSettingsChange: (settings: SkinSmoothingSettings) => void;
  onProcess: () => void;
  onReset: () => void;
...
export const SkinSmoothingSettingsPanel: React.FC<SkinSmoothingSettingsPanelProps> = ({ settings, onSettingsChange, onProcess, onReset, isProcessing, hasImage }) => {
  return (
    <aside className="w-80 bg-slate-800/50 border-l border-slate-700/50 flex flex-col flex-shrink-0">
      <div className="p-4 border-b border-slate-700/50">
        <h2 className="text-lg font-bold text-white">Làm mịn da</h2>
        <p className="text-xs text-slate-400 mt-1">Xóa mụn, nếp nhăn và làm đều màu da mà vẫn giữ được kết cấu tự nhiên.</p>
      </div>
      <div className="flex-1 p-4 space-y-6 overflow-y-auto">
        <div>
          <label htmlFor="level" className="block text-sm font-medium text-slate-300 mb-2">
            Mức độ làm mịn: <span className="font-bold text-blue-400">{settings.level}</span>
          </label>
          <input
            id="level"
            type="range"
      ...
      <div className="p-4 border-t border-slate-700/50 space-y-3">
        ...
        >
          {isProcessing ? 'Đang xử lý...' : 'Làm mịn da'}
        </button>
        <button
          onClick={onReset}

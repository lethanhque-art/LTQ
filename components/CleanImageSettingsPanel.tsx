...
import { type CleanImageSettings } from '../types';

interface CleanImageSettingsPanelProps {
  settings: CleanImageSettings;
  onSettingsChange: (settings: CleanImageSettings) => void;
  onProcess: () => void;
  onReset: () => void;
...
export const CleanImageSettingsPanel: React.FC<CleanImageSettingsPanelProps> = ({ settings, onSettingsChange, onProcess, onReset, isProcessing, hasImage }) => {
  return (
    <aside className="w-80 bg-slate-800/50 border-l border-slate-700/50 flex flex-col flex-shrink-0">
      <div className="p-4 border-b border-slate-700/50">
        <h2 className="text-lg font-bold text-white">Làm sạch ảnh</h2>
        <p className="text-xs text-slate-400 mt-1">Loại bỏ nhiễu, hạt và các vệt nén (artifacts) khỏi ảnh để làm cho nó trong và sạch hơn.</p>
      </div>
      ...
      <div className="p-4 border-t border-slate-700/50 space-y-3">
        ...
        >
          {isProcessing ? 'Đang xử lý...' : 'Làm sạch ảnh'}
        </button>
        <button
          onClick={onReset}
